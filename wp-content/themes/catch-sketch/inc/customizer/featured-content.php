...
function catch_sketch_featured_content_options( $wp_customize ) {
    ...
    catch_sketch_register_option( $wp_customize, array(
            ...
            'label'             => sprintf( esc_html__( 'For all Featured Content Options for this Theme, go %1$shere%2$s', 'catch-sketch' ),
    ...
    $wp_customize->add_section( 'catch_sketch_featured_content', array(
			'title' => esc_html__( 'Featured Content', 'catch-sketch' ),
    ...
    catch_sketch_register_option( $wp_customize, array(
            'name'              => 'catch_sketch_featured_content_etc_note',
            'sanitize_callback' => 'sanitize_text_field',
            'custom_control'    => 'Catch_Sketch_Note_Control',
            'active_callback'   => 'catch_sketch_is_ect_featured_content_inactive',
            /* translators: 1: <a>/link tag start, 2: </a>/link tag close. */
            'label'             => sprintf( esc_html__( 'For Featured Content, install %1$sEssential Content Types%2$s Plugin with Featured Content Type Enabled', 'catch-sketch' ),
	...
	catch_sketch_register_option( $wp_customize, array(
			...
			'default'           => 'disabled',
			'active_callback'   => 'catch_sketch_is_ect_featured_content_active',
	...
	catch_sketch_register_option( $wp_customize, array(
			...
			'active_callback'   => 'catch_sketch_is_featured_content_active',
			'description'       => esc_html__( 'Save and refresh the page if No. of Featured Content is changed (Max no of Featured Content is 20)', 'catch-sketch' ),
			'input_attrs'       => array(
				'style' => 'width: 100px;',
				'min'   => 0,
			),
	...
	for ( $i = 1; $i <= $number ; $i++ ) {
		...
		catch_sketch_register_option( $wp_customize, array(
				...
				'active_callback'   => 'catch_sketch_is_featured_content_active',
				'label'             => esc_html__( 'Featured Content', 'catch-sketch' ) . ' ' . $i ,
				'section'           => 'catch_sketch_featured_content',
				'type'              => 'select',
...
if ( ! function_exists( 'catch_sketch_is_featured_content_active' ) ) :
	...
	*/
	function catch_sketch_is_featured_content_active( $control ) {
		...
		return ( catch_sketch_is_ect_featured_content_active( $control ) &&  catch_sketch_check_section( $enable ) );
	}
endif;

if ( ! function_exists( 'catch_sketch_is_ect_featured_content_active' ) ) :
    /**
    * Return true if featured_content is active
    *
    * @since Catch Sketch 1.0
    */
    function catch_sketch_is_ect_featured_content_active( $control ) {
        return ( class_exists( 'Essential_Content_Featured_Content' ) || class_exists( 'Essential_Content_Pro_Featured_Content' ) );
    }
endif;

if ( ! function_exists( 'catch_sketch_is_ect_featured_content_inactive' ) ) :
    /**
    * Return true if featured_content is active
    *
    * @since Catch Sketch 1.0
    */
    function catch_sketch_is_ect_featured_content_inactive( $control ) {
        return ! ( class_exists( 'Essential_Content_Featured_Content' ) || class_exists( 'Essential_Content_Pro_Featured_Content' ) );
    }
endif;
